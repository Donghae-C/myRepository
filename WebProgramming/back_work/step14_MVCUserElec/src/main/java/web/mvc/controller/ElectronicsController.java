...
public class ElectronicsController implements Controller {
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response) throws SQLException {
		...
		if(pageNo <= 0) {
			throw new SQLException("페이지가 없습니다.");
		}
		List<Electronics> elecList = eService.selectAll(pageNo);
	...
	public ModelAndView update(HttpServletRequest request, HttpServletResponse response) throws ServletException, SQLException, AuthenticationNotSupportedException {
		String modelNum = request.getParameter("modelNum");
		...
		int p = Integer.parseInt(price);
		Electronics elec = new Electronics(modelNum, modelName, p, description, password);
		eService.update(elec);
		
		return new ModelAndView("front", true);
	}
	
	public ModelAndView delete(HttpServletRequest request, HttpServletResponse response) throws SQLException, AuthenticationNotSupportedException {
		String modelNum = request.getParameter("modelNum");
		String password = request.getParameter("password");
		Electronics selected = eService.selectByModelnum(modelNum, false);
		if(!password.equals(selected.getPassword())) {
			throw new AuthenticationNotSupportedException("비밀번호가 맞지 않음");
		}
		ServletContext application = request.getServletContext();
		String saveDir = (String)application.getAttribute("saveDir");
		eService.delete(modelNum, password, saveDir + "/" + selected.getFname());
		return new ModelAndView("front", true);
	}
}
