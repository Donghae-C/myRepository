...
select * , if(kor>=80,"합격","불합격") as 합격여부 from report;

-- EX) 성적테이블에서 국어점수가 80이상이면 합격, 아니면 불합격  합격여부 필드를 만든다. - CASE END 로사용
select * , case when kor >=80 then '합격' else '불합격' end as '합격여부' from report;
...
select name, ifnull(kor,0) + ifnull(eng,0) + ifnull(math, 0) sum, ifnull(kor,0) + ifnull(eng,0) + ifnull(math, 0) / 3 avg from report;

--  국어총점, 국어평균을 검색해보자.
select sum(kor) 국어총점, avg(kor) 국어평균1, avg(ifnull(kor, 0)) 국어평균2, max(kor), min(kor), count(kor), count(*) from report;
select max(kor), min(kor) from report;

-- 일반컬럼과 집계함수를 SELECT절에 함께 쓸수 있을까?? -> 안된다!!!
select ban, sum(kor), avg(kor), avg(ifnull(kor, 0)), count(kor), max(kor), min(kor) from report group by ban order by ban;
...
select ban, max(kor), min(kor), sum(kor), avg(kor), count(kor) from report where kor >= 70 group by ban order by ban;
 
-- KOR의 평균이 80 이상인 학생들의 반별 국어 최대, 최소 총점 평균 인원수 
select ban, max(kor), min(kor), sum(kor), avg(kor), count(kor) from report group by ban having avg(kor) >= 80 order by ban;

select job, ename, sum(sal), avg(sal), max(sal), min(sal) from emp group by rollup(job, ename);
...
-- 서브쿼리 이용
select * from (SELECT row_number() OVER(ORDER BY SAL)  as no , EMPNO, ENAME, JOB, SAL from emp) as t where no <=3;

-- limit 이용
select *, row_number() over() from emp order by sal limit 3;
