...
select * from subemp where sal > all (select sal from subemp where dept_id = 100);
/*ex3)급여를 3000이상 받는 사원이 소속된 부서와 
   동일한 부서에서 근무하는 사원들의 정보*/
select * from subemp;
select * from subemp where dept_id in (select dept_id from subemp group by dept_id having max(sal)>=3000);
 /* ex4) 부서번호가 300인 사원들중에서 급여를 가장 많이 받는 사원보다
      더 많은 급여를 받는 사람의 정보를 검색.*/
select * from subemp where sal > all(select sal from subemp where dept_id = 300);
/* ex5) 부서번호가 300인 사원들중에서 급여를 가장 적게 받는 사원보다
     더 많은 급여를 받는 사람의 정보를 검색*/
select * from subemp where sal > any(select sal from subemp where dept_id = 300);
...
select e.dept_id, d.dname from subemp e join subdept d using(dept_id) where e.job = '사무직';
select dept_id, dname from subdept where dept_id in (select dept_id from subemp where job = '사무직');
-- ex8) 부서가 경리부인 모든 사원의 정보출력
select * from subemp e join subdept d using(dept_id) where d.dname = '경리부';
...
/*ex10) 이름에 '정'이 들어가면서 평균급여보다 높은 급여를 받는
     사원과 동일한 부서에서 근무하는 사원의 정보 검색.
      (단, 부서번호 null은 제외함)*/
select * from subemp where dept_id in (select dept_id from subemp where emp_name like '%정%' and sal >(select avg(sal) from subemp));
-- ex11) 각 부서의 어떤 평균 급여보다 급여를 많이 받는 사원의 정보를 검색
select * from subemp where sal > all (select avg(sal) from subemp where dept_id is not null group by dept_id);
-- ex12)  모든 사무직 사원보다 급여가 적으면서 사무직이 아닌 모든 사원의 정보검색
select * from subemp where sal < all (select sal from subemp where job = '사무직') and job <> '사무직';
